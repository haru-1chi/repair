...
try {
    $sql = "SELECT * FROM orderit.depart WHERE depart_name = :depart_name";
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $sql = "INSERT INTO orderit.depart(depart_name) VALUES(:depart_name)";
        $stmt = $conn->prepare($sql);
        $stmt->bindParam(":depart_name", $depart_name);
        ...
        if ($stmt->execute()) {
            // บันทึกข้อมูลสำเร็จ
            $sql = "SELECT * FROM orderit.depart ORDER BY depart_id DESC LIMIT 1";
